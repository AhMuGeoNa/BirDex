...
int_port=$(cat ${CONFIG_PATH} | jq '.birdex_int_port')
check=`ps -ef | grep ${int_port} | wc | awk '{print $1}'`

if [ $check -gt 1 ]; then
    echo "현재 조류 도감이 실행 중 입니다."

else
    
    LOG_PATH=${ROOT_PATH}/logs/BirDex
    ...
    LOG_PATH=${LOG_PATH}/${today}_birdex_${idx}.log
    echo 이번에 생성할 로그 파일 경로 입니다. ${LOG_PATH}

    echo "\nssl 서버로 장고를 실행합니다."
    nohup python $ROOT_PATH/BirDex/manage.py runserver  0:${int_port} > $LOG_PATH &
